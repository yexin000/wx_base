var Showbo = {
	author : 'showbo',
	homepage : 'http://www.code-design.cn'
};
// 是否为ie浏览器
...
Showbo.getScrollHeight = function() {
	var h = !Showbo.isStrict ? document.body.scrollHeight
...
Showbo.getClientWidth = function() {/* if(Showbo.IsIE) */
	return Showbo.isStrict ? document.documentElement.clientWidth
			: document.body.clientWidth;/* else return self.innerWidth; */
}
Showbo.getScrollWidth = function() {
	var w = !Showbo.isStrict ? document.body.scrollWidth
...
Showbo.Msg = {
	...
	InitMsg : function(width) {
		...
		var ifStr = '<iframe src="javascript:false" mce_src="javascript:false" style="position:absolute; visibility:inherit; top:0px;left:0px;width:100%; height:100%; z-index:-1;' + 'filter=\'progid:DXImageTransform.Microsoft.Alpha(style=0,opacity=0)\';"></iframe>', html = '<div class="top"><div class="rightAlert"><div class="title" id="dvMsgTitle">标题</div></div></div>' + '<div class="body"><div class="rightAlert"><div class="ct" id="dvMsgCT"><textarea></textarea></div></div></div>' + '<div class="bottom" id="dvMsgBottom"><div class="right"><div class="btn" id="dvMsgBtns"></div></div></div>';
	...
	show : function(cfg) {
		...
		// 检查是否指定高度
		if (cfg.width)
			this.defaultHeight = cfg.height;
		this.dvMsgBox.style.width = this.defaultHeight + 'px';
		...
		if (cfg.wait) {
			...
			if (cfg.buttons.yes) {
				this.dvBtns.appendChild(this.createBtn('yes', cfg.buttons.yes,
						cfg.fn));
				if (cfg.buttons.no)
					this.dvBtns.appendChild(document.createTextNode('　'));
			}
			if (cfg.buttons.no)
				this.dvBtns.appendChild(this.createBtn('no', cfg.buttons.no,
						cfg.fn));
		}
	...
	onResize : function(isResize) {
		...
		Showbo.Msg.dvMsgBox.style.top = document.documentElement.scrollTop
				+ Math
						.floor((Showbo.BodyScale.y - Showbo.Msg.dvMsgBox.offsetHeight) / 2)
				+ 'px';
		Showbo.Msg.dvMsgBox.style.left = Math
				.floor((Showbo.BodyScale.x - Showbo.Msg.dvMsgBox.offsetWidth) / 2) + 'px';
	}
}
