var YDataGrid = function(config){
		...
		var Handler = {
			...
			edit:  function(callback){
				...
				if (Utils.checkSelectOne(record)){
					...
					WeiXin.getById(Action.getId,data,function(result){
			...
			remove: function(callback){
				...
				if (Utils.checkSelect(records)){
					$.messager.confirm('提示','您确定想删除该记录?',function(r){  
					    if (r){
					    	WeiXin.progress();
					    	...
					    	var data = arr.join("&");
					   		WeiXin.deleteForm(Action.remove,data,function(result){
								WeiXin.closeProgress();
								Events.refresh();
								//回调函数
								if(jQuery.isFunction(callback)){
			...
			save: function(callback){
				if(Form.edit.form('validate')){
					...
					WeiXin.saveForm(Form.edit,function(data){
						WeiXin.closeProgress();
			...
			close : function (callback){
				$.messager.confirm('提示','您确定想关闭该窗口?',function(r){  
				    if (r){  
				     	Win.edit.dialog('close');
				     	//回调函数
						if(jQuery.isFunction(callback)){
		...
		var Utils = {
			...
			checkSelectOne : function(rows){//检查grid是否只勾选了一行,是返回 true,否返回true
				...
				WeiXin.alert('警告','只能选择一行记录.','warning');  
				return false;
			}
		}
		...
		var initTbar = function(){
			...
			//查询页面授权的btnType
			WeiXin.ajaxJson(_url,data,function(data){
				if(data.success){
					if(data.allType){
