        ...
        config:{
            ...
            dataGrid:{
                ...
                columns:[[
                    ...
                    {field:'activityName',title:'活动名称',width:120,align:'center',sortable:true},
                    {field:'activityMoney',title:'活动金额',align:'center',width:240,sortable:true},
                    ...
                    {field:'opts',title:'操作',width:220,align:'left',formatter:function(value,row,index){
                            var html ="<a href='#' onclick='WeiXin.activity.uploadLogo("+row.id+")'>上传封面</a>";
                            if(null != row.logoPath && "" != row.logoPath) {
                                var viewHtml = "  <a href='#' onclick='WeiXin.activity.showImage(\""+ urls.msUrl + "/"+ row.logoPath +"\")'>查看封面</a>";
                                html += viewHtml
                            }
                            if(row.auditStatus == 0) {
                                var auditHtml = " <a href='#' onclick='WeiXin.activity.audit("+row.id+")'>审核通过 </a>"
                                auditHtml += " <a href='#' onclick='WeiXin.activity.auditDeny("+row.id+")'> 审核不通过</a>";
                                html += auditHtml;
                            }
                            return html;
                        }
