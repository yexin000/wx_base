$package('WeiXin.sysUser');
WeiXin.sysUser = function(){
	var _box = null;
	var _this = {
		...
		savePwd:function(){
			...
			if(_this.editPwdForm().form('validate')){
				...
				WeiXin.saveForm(_this.editPwdForm(),function(data){
					WeiXin.closeProgress();//关闭缓冲条
		...
		initForm:function(){
			...
			_this.editPwdWin().find("#btn-pwd-close").click(function(){	
				$.messager.confirm('提示','您确定要关闭该窗口?',function(r){  
				    if (r){  
				     	_this.editPwdWin().dialog('close');
				    }  
				});
		...
		config:{
  			dataGrid:{
				...
				toolbar:[
					...
					{id:'btnedit',text:'编辑密码',btnType:'editPwd',iconCls:'icon-edit',handler:function(){
							var selected = _box.utils.getCheckedRows();
							if ( _box.utils.checkSelectOne(selected)){
								_this.editPwdForm().resetForm();
								_this.editPwdForm().find("input[name='id']").val(selected[0].id);
								_this.editPwdForm().find("input[name='username']").val(selected[0].username);
