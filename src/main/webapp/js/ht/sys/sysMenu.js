$package('WeiXin.sysMenu');

WeiXin.sysMenu = function(){
	var _box = null;
	var _this = {
		...
		addLine: function(data){
			...
			$(".remove-btn",line).click(function(){
				WeiXin.confirm('提示','您确定要删除此记录?',function(r){
					if(r){
						_this.delLine(line);
					}
				})
  			...
  			dataGrid:{
  				title:'菜单列表',
	   			...
	   			columns:[[
					...
					{field:'url',title:'访问地址',width:220,sortable:true},
					...
					{field:'childMenus',title:'子菜单',width:120,align:'center',formatter:function(value,row,index){
						var html ="<a href='#' onclick='WeiXin.sysMenu.toList("+row.id+")'>子菜单管理("+row.subCount+")</a>";
						return html;
					}}
				]],
				toolbar:[
